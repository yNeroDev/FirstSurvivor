...
func on_health_changed():
	if hit_flash_tween != null && hit_flash_tween.is_valid():
		hit_flash_tween.kill() # is it there and still running? Kill it.
	...
	.set_ease(Tween.EASE_IN).set_trans(Tween.TRANS_CUBIC) # lerp the white shade from .9 to 0 in 0.2 s
